...
async function runTest() {
    console.log('🚀 Starting Nested Stack Leak Reproduction...\n');
    ...
    try {
        ...
        // 1. Log an error with a nested stack trace
        const sensitiveStack = 'Error: Boom\n    at User.secrets (user.js:42)';
        Logger.error('Test Nested Error', { error: { message: 'Boom', stack: sensitiveStack } });
        ...
        if (lastError.context && lastError.context.error && lastError.context.error.stack === sensitiveStack) {
            console.log('⚠️ VULNERABILITY CONFIRMED: Nested stack trace persisted in localStorage');
        } else {
            console.log('✅ SAFE: Nested stack trace NOT persisted');
        }
