...
## 2026-01-23 - Stack Trace Exposure in localStorage Error Logging
**Vulnerability:** Error handlers logged complete stack traces to `localStorage` via the Logger module. Stack traces were persisted in two locations: `app.js:597` (session save errors) and `observability.js:173` (global error handler). These traces contained sensitive system information including file paths, function names, and internal implementation details accessible to any XSS exploit or malicious browser extension.
**Learning:** In offline-first PWAs, localStorage is both persistent *and* accessible to client-side code. While stack traces are valuable for debugging, persisting them client-side creates an information disclosure vulnerability. The principle: "Server logs are privileged; client logs are public." This is especially critical when error logging happens automatically for all uncaught exceptions.
**Prevention:** Never persist stack traces to client-accessible storage (localStorage, IndexedDB, cookies). Log error messages, locations (file/line), and sanitized context only. For production debugging, implement server-side error reporting (e.g., Sentry) or use browser DevTools. If client-side error persistence is required, strip stack traces and limit retention to prevent data accumulation.
